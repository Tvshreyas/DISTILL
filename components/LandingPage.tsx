  ...
  return (
    <header className="px-6 pt-6 pb-0 flex items-center justify-between max-w-6xl mx-auto w-full relative z-[60]">
      <div className="flex items-center gap-3">
        ...
        <span className="font-grotesk font-black text-2xl tracking-tight lowercase">distill</span>
      </div>
      
      {/* Keeping a minimal link to sign in so users aren't stranded */}
      <Link href="/sign-in" className="text-sm font-bold uppercase tracking-widest hover:text-peach transition-colors">
        log in
      </Link>
    </header>
  );
...
function Hero() {
  return (
    <section className="relative pt-0 pb-32 px-6 overflow-hidden">
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 -z-20 opacity-[0.04] select-none pointer-events-none" aria-hidden="true">
        <span className="text-[50vw] font-grotesk font-black text-soft-black leading-none block">D</span>
      </div>
      ...
      <div className="max-w-6xl mx-auto text-center relative z-10 -mt-12 md:-mt-24">
        ...
        <Sticker label="unplug" icon="🔌" className="top-[10%] left-2 rotate-[-12deg] hidden xl:block" />
        <Sticker label="breathe" icon="✨" className="top-[15%] right-2 rotate-[8deg] hidden xl:block" />
        <Sticker label="compounds" icon="📈" className="bottom-[40%] left-10 rotate-[15deg] hidden xl:block" />
        <Sticker label="archive" icon="🗄️" className="bottom-[20%] right-10 rotate-[-8deg] hidden xl:block" />
      </div>
    </section>
  );
}

function Sticker({ label, icon, className }: { label: string, icon: string, className?: string }) {
  return (
    <motion.div
...
function SlantedMarquee() {
  return (
    <div className="relative py-12 -rotate-1 scale-105 overflow-hidden z-20" aria-hidden="true">
...
function BentoGrid() {
  return (
    <section id="features" className="py-32 px-6 bg-white/30 relative" aria-labelledby="features-heading">
      <div className="max-w-7xl mx-auto space-y-24">
        <header className="max-w-2xl">
          <h2 id="features-heading" className="font-grotesk text-5xl font-bold leading-none lowercase">
            built for <br />
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
          <BentoCard className="md:col-span-7 bg-sage" title="session starts" description="open a cognitive loop before you consume. Distill creates a space for your brain to stay active." label="01" />
          <BentoCard className="md:col-span-5 bg-lavender" title="reflection capture" description="write your own perspective immediately after finishing. no AI summaries, just your raw thoughts." label="02" />
          <BentoCard className="md:col-span-5 bg-white" title="personal library" description="a searchable, private archive of your own mind that compounds and grows with every session." label="03" />
          <BentoCard className="md:col-span-7 bg-peach" title="spaced resurfacing" description="your past reflections resurface at timed intervals, forcing you to engage with your former self." label="04" />
        </div>
      </div>
    </section>
  );
...
function BentoCard({ className, title, description, label }: { className: string, title: string, description: string, label: string }) {
  return (
    <motion.div
    ...
    >
      <div className="flex justify-between items-start">
        <span className="font-grotesk font-bold text-4xl leading-none opacity-20" aria-hidden="true">{label}</span>
        <div className="w-12 h-12 rounded-full border border-soft-black/10 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity" aria-hidden="true">
          &rarr;
        </div>
      </div>
      <div>
        <h3 className="font-grotesk text-3xl font-bold mb-3 lowercase">{title}</h3>
        <p className="text-muted-text font-medium leading-relaxed">{description}</p>
      </div>
    </motion.div>
  );
}
  ...
  return (
    <section className="py-32 px-6" aria-labelledby="faq-heading">
      <div className="max-w-3xl mx-auto space-y-12">
        <h2 id="faq-heading" className="font-grotesk text-4xl font-bold text-center lowercase">questions you might have</h2>
        <div className="space-y-4">
          <FAQItem question="is this just another journaling app?" answer="not quite. distill is a thinking development tool. it helps you build a reflection habit and develop your own perspective on what you consume." />
          <FAQItem question="how does it help with focus?" answer="by providing a high-contrast interface that encourages active thinking, helping you engage with content instead of just scrolling past it." />
          <FAQItem question="can i export my thoughts?" answer="yes, your perspectives are yours. you can export them as clean markdown or json at any time." />
        </div>
      </div>
    </section>
  );
  ...
  return (
    ...
    >
      <summary className="p-8 flex items-center justify-between font-bold text-lg list-none font-grotesk lowercase">
        <span>{question}</span>
        <span className={`w-8 h-8 rounded-full bg-sage/50 flex items-center justify-center transition-transform ${isOpen ? 'rotate-45' : ''}`}>
          +
        </span>
      </summary>
  ...
  return (
    <section className="py-32 px-6 relative overflow-hidden">
       ...
       <div className="max-w-5xl mx-auto text-center space-y-12">
          ...
          <div className="pt-8">
            <Link href="/sign-up" className="brutal-btn bg-soft-black text-white text-xl px-12 py-5">
...
function Footer() {
  return (
    <footer className="pt-32 pb-12 px-6 bg-white border-t-8 border-soft-black overflow-hidden relative">
      <div className="absolute -bottom-10 left-0 w-full opacity-[0.03] select-none pointer-events-none" aria-hidden="true">
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-12 relative z-10">
        ...
        <div>
          <ul className="space-y-3 text-sm text-muted-text font-medium">
            ...
            <li><Link href="/dashboard">web app</Link></li>
          </ul>
        </div>
        <div>
      ...
      <div className="max-w-7xl mx-auto mb-20 p-8 rounded-[2rem] border-2 border-soft-black/20 bg-sage/5">
        <p className="text-xs text-muted-text font-medium leading-relaxed italic text-center max-w-4xl mx-auto">
          distill is a thinking development and reflection tool. it is NOT a medical device and is NOT intended to diagnose, treat, cure, or prevent anxiety, digital addiction, attention disorders, or any other medical or psychological condition. users experiencing mental health difficulties should consult a qualified healthcare professional.
        </p>
      </div>
