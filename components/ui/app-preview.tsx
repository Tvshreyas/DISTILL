        ...
        <div className="flex-1 flex">
          ...
          <div className="w-20 md:w-64 border-r-2 border-soft-black/5 p-6 space-y-8 hidden md:block">
            ...
            <div className="space-y-4">
              {[1, 2, 3, 4].map((i) => (
                <div key={i} className="h-4 w-full bg-soft-black/5 rounded-full" />
              ))}
            </div>
          </div>
            ...
            <div className="relative">
              <div className="brutal-card bg-warm-bg border-2 border-dashed border-soft-black/20 p-8 space-y-6">
                ...
                <div className="flex gap-4">
                  <div className="px-4 py-2 rounded-full border border-soft-black/10 text-[10px] font-bold uppercase tracking-wider text-muted-text">perspective</div>
                  <div className="px-4 py-2 rounded-full border border-soft-black/10 text-[10px] font-bold uppercase tracking-wider text-muted-text">layer 01</div>
                </div>
              </div>
              ...
              >
                 <span className="font-grotesk font-bold text-xs">active recall</span>
              </motion.div>
            </div>
            ...
            <div className="pt-4">
              <div className="flex justify-between items-center mb-4 px-2">
                 ...
                 <div className="flex gap-1">
                   {[1,2,3,4,5].map(i => <div key={i} className="w-2 h-2 rounded-sm bg-peach/40" />)}
                 </div>
              </div>
                ...
                {Array.from({ length: 30 }).map((_, i) => (
                   <motion.div 
                     ...
                     className={`aspect-square rounded-full border border-soft-black/5 ${
                       i % 7 === 0 || i % 7 === 2 ? 'bg-peach shadow-[2px_2px_0px_#292524]' : i % 5 === 0 ? 'bg-sage/40' : 'bg-transparent'
                     }`}
                   />
                ))}
              </div>
