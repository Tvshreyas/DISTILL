...
describe("sanitizeContent", () => {
  ...
  it("strips <script> tags and their content", () => {
    ...
    expect(sanitizeContent('before<script>alert("xss")</script>after')).toBe(
      "beforeafter",
    );
  });
  ...
  it("strips onerror and other event handler attributes", () => {
    ...
    expect(sanitizeContent('<div onmouseover="alert(1)">hover</div>')).toBe(
      "hover",
    );
  });

  it("strips javascript: protocol in href", () => {
    expect(sanitizeContent('<a href="javascript:alert(1)">click</a>')).toBe(
      "click",
    );
  });

  it("strips nested XSS payloads", () => {
    expect(sanitizeContent("<svg><script>alert(1)</script></svg>")).toBe("");
