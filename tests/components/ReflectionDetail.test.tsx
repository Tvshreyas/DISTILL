...
function makeReflection(
  overrides: Partial<Doc<"reflections">> = {},
): Doc<"reflections"> {
  return {
...
describe("ReflectionDetail", () => {
  it("renders the reflection content wrapped in smart quotes", () => {
    render(
      <ReflectionDetail
        reflection={makeReflection({ content: "Test content" })}
      />,
    );
  ...
  it("renders null when reflection is falsy", () => {
    const { container } = render(
      <ReflectionDetail reflection={null as unknown as Doc<"reflections">} />,
    );
  ...
  it("shows thinking shift rating when present", () => {
    render(
      <ReflectionDetail
        reflection={makeReflection({ thinkingShiftRating: 4 })}
      />,
    );
  ...
  it("hides thinking shift rating when null", () => {
    render(
      <ReflectionDetail
        reflection={makeReflection({ thinkingShiftRating: undefined })}
      />,
    );
  ...
  it("renders long content without truncation", () => {
    ...
    render(
      <ReflectionDetail
        reflection={makeReflection({ content: longContent })}
      />,
    );
