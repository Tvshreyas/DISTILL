# Social Content Strategy — Distill

*Created: 2026-03-15*
*Launch: Tuesday March 17, 2026*

---

## Platform Focus

**Primary:** X/Twitter — tech/reading/productivity audience, real-time engagement, thread-friendly
**Secondary:** LinkedIn — professional readers, book clubs, knowledge workers
**Tertiary:** Reddit — r/productivity, r/books, r/selfimprovement (launch posts already drafted in LAUNCH-STRATEGY.md)

---

## Content Pillars

| Pillar | % | Description |
|---|---|---|
| Thinking provocations | 40% | Questions and observations about reading, consuming, and thinking |
| Building in public | 25% | Real numbers, lessons, decisions, founder perspective |
| Content repurposing | 20% | Blog post takeaways, glossary concepts, reflection prompts |
| Product | 15% | Feature highlights, user milestones, updates |

---

## Brand Voice on Social

- Lowercase. No exclamation marks. Periods at the end of thoughts.
- Short sentences. Line breaks between ideas.
- Never: praise, medical claims, hustle culture, "10x", "game-changer", "unlock your potential"
- Always: direct, factual, identity-framing ("a thinking tool"), slightly contrarian
- Tone reference: @naval meets @visakanv — compressed insight, no hype

---

## Week 1 Content Calendar (Launch Week: March 17-23)

### Pre-Launch (already in LAUNCH-STRATEGY.md)
- **Saturday Mar 15:** "i built a tool because i realized..." tweet
- **Sunday Mar 16:** "the problem isn't that you forget..." tweet
- **Monday Mar 17 pre-dawn:** "unpopular opinion: AI summaries make the reading retention problem worse..." tweet

### Launch Day — Tuesday March 17
- **12:01am PT:** PH goes live, maker comment posted
- **6-7am:** Show HN posted
- **8-9am:** Launch thread (6 tweets, drafts in LAUNCH-STRATEGY.md)
- **2pm:** Building in public update with real numbers
- **8pm:** End of day 1 tweet with signup/reflection counts

### Wednesday March 18
**Morning:**
> you read 3 articles today. what did you actually think about any of them?
>
> not what the author said. what YOU thought.
>
> that gap — between consuming and thinking — is the entire problem.

**Afternoon (building in public):**
> day 2 numbers:
> [X] signups. [X] reflections written.
> one thing i didn't expect: [specific observation from data].

### Thursday March 19
**Morning:**
> highlights don't mean you understood something.
> bookmarks don't mean you'll come back.
> saving ≠ thinking.
>
> the only proof you processed something is writing what you thought about it.

**Afternoon (repurpose from blog):**
> from the distill blog: you forget 90% of what you read within a week.
>
> the forgetting curve is real. the only reliable counter: active retrieval.
>
> writing a reflection forces retrieval. it's the simplest version of this.
>
> full post: distillwise.com/blog/why-you-forget-everything-you-read

### Friday March 20
**Morning:**
> every productivity tool adds complexity.
> folders. tags. databases. second brains.
>
> distill removes it. one input: what did you think?
> no org system. no taxonomy. just your perspective, captured.

**Afternoon (engagement question):**
> genuine question: what's the last book you read where you can still articulate what YOU thought about it?
>
> not the summary. not the author's argument. your perspective.
>
> reply with the book. curious.

### Saturday March 21
**Morning (thinking provocation):**
> you spent 4 hours on your phone today.
> how many of your own thoughts did you have during that time?
>
> consuming isn't thinking. scrolling isn't processing.
> the difference matters.

### Sunday March 22
**Morning (weekend reflection):**
> sunday thought: the people who think clearly aren't consuming less.
> they're processing more.
>
> same inputs, different output. the variable is reflection.

---

## Week 2-4 Content Templates

### Thinking Provocations (use 3x/week)

> how many books have you read this year?
> now: how many original thoughts did you form about any of them?
> the gap between those two numbers is the problem.

> unpopular opinion: reading more books isn't the goal.
> thinking about fewer books is.

> your notes app has 2,000 entries.
> how many of them are YOUR thoughts vs. someone else's ideas you copied?

> the last podcast you listened to — can you articulate one thing you disagreed with?
> if not, you consumed it. you didn't process it.

> AI can summarize a book in 30 seconds.
> it still can't tell you what you think about it.
> that's the part that matters.

> "i read 50 books this year" is not an accomplishment.
> "i changed my mind about 3 things this year because of what i read" is.

> speed reading is optimizing for the wrong metric.
> comprehension without reflection is just faster forgetting.

> you don't have an information problem. you have a processing problem.
> the bottleneck isn't input. it's what you do after.

### Building in Public (use 2x/week)

> week [X] of distill:
> [X] users. [X] reflections written.
> most common content type: [books/articles/podcasts].
> [one specific observation or surprise].

> something i learned building distill:
> [genuine lesson — technical, product, or personal].

> honest update: [metric] is [good/bad].
> here's what i'm doing about it: [action].
> building in public means sharing the real numbers, not just the good ones.

> feature request i keep getting: [request].
> why i'm not building it yet: [reason].

### Content Repurposing (use 2x/week)

**From blog posts:**
> from the distill blog: [key insight from post]
>
> [2-3 sentence expansion]
>
> full post: distillwise.com/blog/[slug]

**From glossary:**
> what is [concept]?
>
> [2-sentence definition from glossary]
>
> most people [common mistake]. [better approach].
>
> more: distillwise.com/glossary/[slug]

**From reflection guides:**
> how to reflect after [content type]:
>
> 1. [step 1]
> 2. [step 2]
> 3. [step 3]
>
> full guide: distillwise.com/reflect/[slug]

### Product (use 1x/week, max)

> distill now has [feature/milestone].
> [one sentence on what it means for users].
> distillwise.com

> [X] reflections have been written in distill.
> every one of those is someone choosing to think instead of scroll.

---

## LinkedIn Content (1-2x/week)

LinkedIn posts should be longer (150-300 words), more professional, but same voice.

### Template 1: Problem Statement
> I read 40+ books last year.
>
> When someone asked me what I thought about any of them — not what the author said, but what I thought — I had nothing.
>
> This is the reading paradox: the more you consume, the less you process.
>
> [3-4 sentences expanding on the insight]
>
> I built Distill to solve this for myself. It's a simple tool: after reading/watching/listening to something, you write a short reflection. Your reflections resurface later.
>
> No AI. No summaries. Just: what did you think?
>
> distillwise.com

### Template 2: Data/Insight
> [Data point about reading/retention/attention]
>
> [What this means in practical terms]
>
> [How reflection addresses this]
>
> [Soft CTA to blog post or product]

### Template 3: Contrarian Take
> Unpopular opinion in [knowledge work / productivity / reading]:
>
> [Bold statement]
>
> Here's why: [3-4 sentences of reasoning]
>
> [Connection to thinking/reflection, no hard sell]

---

## Engagement Rules

### Daily (15 min)
- Reply to every comment on your posts
- Reply to every mention/RT
- Comment on 5 posts from accounts in the reading/productivity/building space

### Quality Comments (not "great post!")
- Add a related observation from your experience
- Ask a specific follow-up question
- Respectfully push back with nuance

### Accounts to Engage With
- Founders building in public (IndieHackers community on X)
- Book reviewers and "what I read this month" posters
- Anti-productivity and slow living accounts
- Anyone posting about attention, deep work, or reading retention

---

## Hashtags (Use Sparingly)

X: Generally skip hashtags. They reduce engagement on X.
LinkedIn: 3-5 per post, placed at the bottom: #reading #thinking #productivity #reflection #buildinpublic

---

## Content Repurposing Pipeline

| Source | → X/Twitter | → LinkedIn |
|---|---|---|
| Blog post | 1 thread (3-5 tweets) + 1 standalone insight | 1 long-form post |
| Glossary term | 1 "what is X?" tweet | Skip |
| Reflection guide | 1 "how to" tweet with 3 steps | 1 how-to post |
| User milestone | 1 celebration tweet (no praise, just facts) | Skip |
| Product update | 1 tweet | 1 post if significant |

---

## Metrics

| Metric | Week 1 Target | Month 1 Target |
|---|---|---|
| X followers | +50 | +200 |
| Avg. impressions/tweet | 500 | 1,000 |
| Engagement rate | 3%+ | 4%+ |
| Link clicks to distillwise.com | 20/week | 50/week |
| LinkedIn post impressions | 500/post | 1,000/post |

---

## What NOT to Post

- Screenshots of analytics dashboards (vanity metrics without context)
- "We just hit X users!" without reflection on what it means
- Anything that sounds like an ad: "Try Distill today!", "Sign up now!", "Don't miss out!"
- Emojis in every post (occasional is fine, not a pattern)
- Threads longer than 6 tweets (diminishing engagement after tweet 4)
- Hot takes you don't actually believe (contrarian for clicks)
- Anything that violates copy rules: no praise, no medical claims, no therapeutic framing
